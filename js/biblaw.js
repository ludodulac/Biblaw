(() => {
  ...
  const state = { mode: 'themes', records: [], themes: [], theme: null, sense: '', active: null };
  ...
  async function load() {
    try {
      ...
      state.theme = state.themes.find(r => r.id === 'theme-chouette') || state.themes[0] || null;
      state.records = loaded.filter(r => r.recordType && r.recordType !== 'theme');
  ...
  function summary(r) { return r.recordType === 'psalm' ? (r.id === 'michael-psalm-026' && state.theme ? state.theme.subthemes.map(x => x.summary).join(' ') : r.verses.slice(0, 2).map(v => v.text).join(' ')) : r.summary || (r.text || '').slice(0, 280); }
