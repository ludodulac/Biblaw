#!/usr/bin/env python3
"""Deep descriptive indexing for books 1-2 from the canonical PDF-derived corpus only."""
import json,re
...
def clause(text,label):
 ...
 pos=text.lower().replace('œ','oe').find(label.lower().replace('œ','oe'))
