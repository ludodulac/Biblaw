(() => {
  ...
  const representativePilots = new Set([
    'book-18-psalm-112',
    'book-19-psalm-104',
    'book-20-psalm-105',
    'book-26-psalm-182'
  ...
  const sourcePages = psalm => psalm.source?.pdfPages || psalm.source?.printedPages || (psalm.source?.printedPage ? [psalm.source.printedPage] : []);

  async function load() {
    const bundle = await fetch('data/browser-search-catalog.json').then(r => { if (!r.ok) throw Error('browser-search-catalog'); return r.json(); });
    const docs = bundle.records || [];
    ...
    const selectedPsalms = docs
      .filter(x => x.recordType === 'psalm' && ((x.archangel === 'michael' && x.book?.number === 17) || representativePilots.has(x.id)))
      .sort((a, b) => a.book.number - b.book.number || a.number - b.number || String(a.id).localeCompare(String(b.id), 'fr'));

    selectedPsalms.forEach(psalm => {
      state.records.set(psalm.id, { psalm, prayer: prayers.get(psalm.id), notes: notes.filter(note => note.appliesTo?.recordId === psalm.id) });
    });

    $('recordSelect').innerHTML = [...state.records.values()].map(({ psalm }) => `<option value="${esc(psalm.id)}">${angel(psalm.archangel)} · Livre ${psalm.book.number} · Psaume ${psalm.number} — ${esc(psalm.title)}</option>`).join('');
    $('recordSelect').onchange = () => render($('recordSelect').value);
    $('saveReview').onclick = saveReview;
    $('copyReview').onclick = copyReview;

    const defaultId = 'book-17-psalm-105';
    render(state.records.has(defaultId) ? defaultId : state.records.keys().next().value);
  }

  function render(id) {
    state.active = state.records.get(id);
    if (!state.active) return;
    const { psalm, prayer, notes } = state.active, pages = sourcePages(psalm);
    $('recordSelect').value = id;
    $('sourceLink').href = `${pdf}#page=${pages[0] || 1}`;
    ...
    $('recordHeader').innerHTML = `<div class="record-title"><span class="status-pill">${corpusValidated ? 'Validé dans GitHub' : 'À valider humainement'}</span><h2>${angel(psalm.archangel)} · Psaume ${psalm.number}</h2><p>Livre ${psalm.book.number} — ${esc(psalm.book.title)} · ${pages.length ? `pages ${pages.join('–')}` : 'pages non renseignées'}</p><h3>${esc(psalm.title)}</h3></div>`;
    ...
    $('checklist').innerHTML = checks.map(([checkId, checkLabel]) => `<label class="check-row"><input type="checkbox" data-check="${checkId}" ${localStorage.getItem(key(checkId)) === '1' || (localStorage.getItem(key(checkId)) === null && corpusValidated) ? 'checked' : ''}><span>${checkLabel}</span></label>`).join('');
    document.querySelectorAll('[data-check]').forEach(input => input.onchange = () => { $('saveStatus').textContent = 'Modifications non enregistrées.'; });
    $('reviewNote').value = localStorage.getItem(key('note')) ?? psalm.validation?.review?.note ?? '';
    $('reviewNote').oninput = () => { $('saveStatus').textContent = 'Modifications non enregistrées.'; };
    $('copyStatus').textContent = '';
    const saved = localStorage.getItem(key('savedAt'));
    $('saveStatus').textContent = saved ? `Enregistré sur cet appareil le ${new Date(saved).toLocaleString('fr-FR')}.` : (corpusValidated ? 'Cette validation est déjà inscrite dans GitHub.' : 'Pas encore enregistrée.');
  }

  function saveReview() {
    document.querySelectorAll('[data-check]').forEach(input => localStorage.setItem(key(input.dataset.check), input.checked ? '1' : '0'));
    localStorage.setItem(key('note'), $('reviewNote').value);
    const savedAt = new Date().toISOString();
    localStorage.setItem(key('savedAt'), savedAt);
    $('saveStatus').textContent = `Enregistré sur cet appareil le ${new Date(savedAt).toLocaleString('fr-FR')}.`;
  }

  async function copyReview() {
    const { psalm } = state.active;
    const done = checks.filter(([id]) => localStorage.getItem(key(id)) === '1').map(([, text]) => `✓ ${text}`);
    const todo = checks.filter(([id]) => localStorage.getItem(key(id)) !== '1').map(([, text]) => `□ ${text}`);
    const report = [`Validation — ${angel(psalm.archangel)}, livre ${psalm.book.number}, psaume ${psalm.number}`, ...done, ...todo, '', $('reviewNote').value.trim()].join('\n').trim();
    try {
      await navigator.clipboard.writeText(report);
      $('copyStatus').textContent = 'Compte rendu copié.';
    } catch {
      $('copyStatus').textContent = 'Copie impossible dans ce navigateur.';
    }
  }

  load().catch(error => {
    console.error(error);
    document.querySelector('.validation-grid').innerHTML = '<div class="empty">Impossible de charger les données de validation.</div>';
  });
})();