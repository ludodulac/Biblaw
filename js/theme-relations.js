(() => {
  ...
  const themeIdsForQuery = value => {
    const query = norm(value);
    if (!query) return [];
    const ids = [];
    for (const theme of themesById.values()) {
      if (query === norm(theme.label) || query === norm(theme.id)) ids.push(theme.id);
    }
    return ids;
  };

  const render = () => {
    if (!relationPayload || $('modeExact')?.classList.contains('active')) {
      panel.hidden = true;
      return;
    }
    const currentIds = new Set(themeIdsForQuery($('query')?.value));
    if (!currentIds.size) {
      panel.hidden = true;
      return;
    }
    ...
    for (const relation of relationPayload.relations || []) {
      ...
      let otherThemeId = null;
      if (currentIds.has(relation.sourceThemeId)) {
        currentThemeId = relation.sourceThemeId;
        otherThemeId = relation.targetThemeId;
      } else if (currentIds.has(relation.targetThemeId)) {
        currentThemeId = relation.targetThemeId;
        otherThemeId = relation.sourceThemeId;
      }
      if (!otherThemeId || currentIds.has(otherThemeId) || seen.has(`${otherThemeId}:${relation.relationType}`)) continue;
      const theme = themesById.get(otherThemeId);
      if (!theme) continue;
      seen.add(`${otherThemeId}:${relation.relationType}`);
      related.push({ theme, meta: relationMeta(relation, currentThemeId) });
    }
    ...
    choices.querySelectorAll('[data-validated-theme-id]').forEach(button => {
      button.onclick = () => {
        const theme = themesById.get(button.dataset.validatedThemeId);
        if (!theme) return;
  ...
  async function load() {
    try {
      ...
      themesById = new Map((directory.themes || []).map(theme => [theme.id, theme]));
      render();
    } catch (error) {
      console.error(error);
      panel.hidden = true;
  ...
  document.addEventListener('click', event => {
    if (event.target.closest('[data-related-theme], [data-directory-theme], [data-theme-id]')) setTimeout(render, 0);
  });
  load();
})();
