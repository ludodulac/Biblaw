#!/usr/bin/env python3
"""Deep evidence/indexing pass for books 9-11 using only the canonical PDF-derived corpus.

Theme links are descriptive research aids grounded in exact corpus verses. They are not presented
as an exclusive interpretation and do not claim to exhaust the Psalms' possible levels of reading.
"""
import json,re
...
def process(book):
    path=BOOKS/f'book-{book:02d}.json'; data=json.loads(path.read_text(encoding='utf-8')); grounded_count=curated_count=0
