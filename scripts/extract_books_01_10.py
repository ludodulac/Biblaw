...
def raw_heading_candidates(text: str, first_page: int) -> list[dict]:
    ...
    for i, raw_line in enumerate(lines):
        ...
        if m:
            title = m.group(2).strip(); low = title.lower()
            ...
            for j in range(i + 1, min(i + 4, len(lines))):
                ...
                if not nxt or nxt.startswith("[[PAGE"): continue
...
def add_inventory_candidates(text: str, book: dict, candidates: list[dict]) -> None:
    for item in book.get("psalmHeadingCandidates", []):
        number, page = item.get("number"), item.get("pdfPage")
        if not isinstance(number, int) or not isinstance(page, int): continue
        bounds = page_bounds(text, page)
        if not bounds: continue
        start, end = bounds
        m = re.search(rf"(?m)^\s*{number}\s+(?![.])([^\n]+)$", text[start:end])
        if not m: continue
        candidates.append({"number": number, "title": clean_title(item.get("titleCandidate") or m.group(1)), "offset": start + m.start(), "page": page, "basis": "inventory-page-anchor"})


def verse_one_distance(text: str, offset: int, limit: int = 3500) -> int | None:
    m = re.search(r"(?m)^\s*1\.\s+", text[offset:min(len(text), offset + limit)])
...
def infer_missing_heading(text: str, number: int, lower_offset: int, upper_offset: int | None, first_page: int) -> dict | None:
    ...
    for line in reversed(lines[-10:]):
        ...
        if m:
            ...
            pos = text.rfind(line, max(lower_offset, absolute_reset - 1000), absolute_reset)
            return {"number": number, "title": title, "offset": pos if pos >= 0 else absolute_reset, "page": page_at(text, pos if pos >= 0 else absolute_reset, first_page), "basis": "verse-one-reset-inference"}
    # Never borrow a line carrying another psalm number.
    if lines and not re.match(r"^\d{1,3}\s+", lines[-1]) and len(lines[-1].split()) <= 24 and not lines[-1].endswith((".", ";")):
        title = clean_title(lines[-1])
        pos = text.rfind(lines[-1], max(lower_offset, absolute_reset - 1000), absolute_reset)
        return {"number": number, "title": title, "offset": pos if pos >= 0 else absolute_reset, "page": page_at(text, pos if pos >= 0 else absolute_reset, first_page), "basis": "verse-one-reset-inference"}
    return None
...
def choose_headings(text: str, candidates: list[dict], expected_start: int, first_page: int) -> list[dict]:
    ...
    for h in candidates:
        if h["number"] < expected_start: continue
        if not any(x["offset"] == h["offset"] for x in by_number.get(h["number"], [])):
            by_number.setdefault(h["number"], []).append(h)
    chosen = []; prev_offset = -1; number = expected_start; misses = 0
    ...
    while number <= max_candidate:
        ...
        if options:
            ...
            chosen.append(h); prev_offset = h["offset"]; misses = 0
        else:
            misses += 1
            if chosen and misses >= 4: break
        number += 1
    changed = True
    while changed:
        changed = False; chosen.sort(key=lambda h: h["number"]); nums = [h["number"] for h in chosen]
        for n in range(expected_start, max(nums, default=expected_start - 1) + 1):
            if n in nums or n - 1 not in nums or n + 1 not in nums: continue
            prev = next(h for h in chosen if h["number"] == n - 1); nxt = next(h for h in chosen if h["number"] == n + 1)
            inferred = infer_missing_heading(text, n, prev["offset"], nxt["offset"], first_page)
            if inferred: chosen.append(inferred); changed = True; break
...
for book in selected:
    book_no = book["bookNumber"]; first, last = book["pdfPages"]
    raw_pages = page_text(first, last); archangel = detect_archangel(raw_pages)
    if archangel not in next_expected: raise RuntimeError(f"Unable to determine Archangel for book {book_no}")
    expected_start = next_expected[archangel]
    text = "".join(f"\n[[PAGE {page_no}]]\n{clean_page(book_no, page_no, raw)}" for page_no, raw in raw_pages)
    candidates = raw_heading_candidates(text, first); add_inventory_candidates(text, book, candidates)
    chosen = choose_headings(text, candidates, expected_start, first)
    if not chosen: raise RuntimeError(f"No psalm headings found for book {book_no} from expected {expected_start}")

    book_dir = ROOT / "data/corpus/books" / f"book-{book_no:02d}"; note_dir = ROOT / "data/notes/books" / f"book-{book_no:02d}"
    ...
    extracted_psalms = []; all_notes = []; skipped = []
    for i, h in enumerate(chosen):
        start = h["offset"]; end = chosen[i + 1]["offset"] if i + 1 < len(chosen) else len(text)
        segment = text[start:end]; segment_body = segment.split("\n", 1)[1] if "\n" in segment else ""
        ...
        all_matches = list(re.finditer(r"(?m)^\s*(\d{1,3})\.\s+", body))
        first_one = next((idx for idx, vm in enumerate(all_matches) if int(vm.group(1)) == 1), None)
        if first_one is None:
            skipped.append({"number": h["number"], "title": h["title"], "page": h["page"], "basis": h.get("basis"), "reason": "no-verse-one"}); continue
        verse_matches = all_matches[first_one:]
        verses = []
        for vi, vm in enumerate(verse_matches):
            ...
            if vtext: verses.append({"number": int(vm.group(1)), "text": vtext, "sourcePages": [page_at(body, vm.start(), h["page"])]})
        if not verses:
            skipped.append({"number": h["number"], "title": h["title"], "page": h["page"], "basis": h.get("basis"), "reason": "no-verses"}); continue

        note_ids = []
        for ni, note in enumerate(notes, 1):
            note_id = f"book-{book_no:02d}-psalm-{h['number']:03d}-note-{ni:03d}"; note_ids.append(note_id); all_notes.append(note_id)
            write_json(note_dir / f"{note_id}.json", {"id": note_id, "recordType": "note", "archangel": archangel, "bookNumber": book_no, "appliesTo": {"recordId": f"book-{book_no:02d}-psalm-{h['number']:03d}", "marker": note["marker"], "verse": None}, "text": note["text"], "source": {"document": PDF.name, "pdfPage": note["page"]}, "validation": {"status": "machine-extracted-needs-review"}})
        pages_used = sorted({p for v in verses for p in v["sourcePages"]})
        write_json(book_dir / f"psalm-{h['number']:03d}.json", {"id": f"book-{book_no:02d}-psalm-{h['number']:03d}", "recordType": "psalm", "archangel": archangel, "book": {"number": book_no, "title": book.get("title")}, "number": h["number"], "title": h["title"], "source": {"document": PDF.name, "pdfPages": pages_used}, "verses": verses, "noteIds": note_ids, "extraction": {"headingBasis": h.get("basis")}, "validation": {"status": "machine-extracted-needs-review", "checks": {"verseCount": len(verses), "verseSequenceStartsAtOne": True}}})
    ...
    if extracted_psalms: next_expected[archangel] = max(p["number"] for p in extracted_psalms) + 1
    write_json(book_dir / "book.json", {"id": f"book-{book_no:02d}", "recordType": "book", "number": book_no, "title": book.get("title"), "archangel": archangel, "source": {"document": PDF.name, "pdfPages": [first, last]}, "psalmIds": [f"book-{book_no:02d}-psalm-{p['number']:03d}" for p in extracted_psalms], "noteIds": all_notes, "numbering": {"expectedStart": expected_start, "nextExpected": next_expected[archangel]}, "validation": {"status": "machine-extracted-needs-review"}})
    report.append({"bookNumber": book_no, "title": book.get("title"), "archangel": archangel, "expectedStart": expected_start, "pdfPages": [first, last], "psalms": extracted_psalms, "skippedHeadings": skipped, "noteCount": len(all_notes)})
