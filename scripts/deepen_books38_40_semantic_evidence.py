#!/usr/bin/env python3
"""Deep evidence pass for books 38-40 using only the canonical PDF-derived corpus."""
import json, re
...
def clause(text, label):
    ...
    before=text[:pos].split(); after=text[pos:].split()
...
def process(book):
    ...
    for analysis in data.get('psalmAnalyses',[]):
        num=analysis['number']; corpus=json.loads((CORPUS/f'book-{book:02d}'/f'psalm-{num:03d}.json').read_text(encoding='utf-8'))
        ...
        for theme in analysis.get('themes',[]):
            ...
            if bad: raise ValueError(f'book {book} psalm {num} theme {theme.get("themeId")}: invalid refs {bad}')
...
def main():
    for book in (38,39,40): process(book)
