...
an exact source-text guard. If it was omitted entirely, no previous Psalm change is made.
...
def extract_case(c):
 ...
 nums=[v['number'] for v in verses]; expected=list(range(c['startVerse'],max(nums,default=c['startVerse']-1)+1))
 ...
 if c['previous'] is not None:
  ...
  if prev_path.exists():
   prev=json.loads(prev_path.read_text(encoding='utf-8')); pv=prev.get('verses',[])
   idx=next((i for i,v in enumerate(pv) if v.get('number')==c['startVerse']),None)
   if idx is not None:
...
def repair_book44_final_psalm():
 ...
 v16['text']=v16['text'][:marker.start()].strip(); data['verses']=[v for v in verses if isinstance(v.get('number'),int) and 1<=v['number']<=16]
 by={}
 for v in data['verses']: by.setdefault(v['number'],v)
 data['verses']=[by[n] for n in sorted(by)]; nums=[v['number'] for v in data['verses']]
 if nums!=list(range(1,17)): raise RuntimeError(f'Unexpected repaired book44 Psalm285 numbering: {nums}')
 data['noteIds']=[]; data['source']['pdfPages']=sorted({p for v in data['verses'] for p in v.get('sourcePages',[])})
 data.setdefault('extraction',{})['auditedFinalAnnexDetached']=True; data.setdefault('validation',{})['status']='machine-extracted-source-boundary-audited'; data['validation']['checks']={'verseCount':16,'verseSequenceStartsAtOne':True}; write(path,data)
 meta_path=CORPUS/'book-44'/'book.json'; meta=json.loads(meta_path.read_text(encoding='utf-8')); meta['noteIds']=sorted(x for x in set(meta.get('noteIds',[])) if not x.startswith('book-44-psalm-285-note-')); write(meta_path,meta)
