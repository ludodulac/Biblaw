#!/usr/bin/env python3
"""Deep descriptive indexing for books 6-8 from the canonical PDF-derived corpus only."""
import json,re
...
def clause(text,label):
 ...
 low=text.lower().replace('œ','oe'); needle=label.lower().replace('œ','oe'); pos=low.find(needle)
...
def process(book):
 ...
 for a in d.get('psalmAnalyses',[]):
  n=a['number']; cp=CORPUS/f'book-{book:02d}'/f'psalm-{n:03d}.json'; co=json.loads(cp.read_text(encoding='utf-8')); verses={v['number']:v.get('text','') for v in co.get('verses',[])}
