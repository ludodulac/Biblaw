(() => {
  ...
  const composeThemeQueryNotions = query => { const terms=norm(query).split(' ').filter(Boolean), has=term=>terms.includes(term), existence=terms.some(term=>['existe','existent','existons','exister','existence'].includes(term)), why=has('pourquoi'), life=has('vivre')||has('vie'); if((why&&existence)||(why&&life)||(has('raison')&&existence))return [new Set(['sens','but']),new Set(['vie'])]; if(has('sens')&&existence)return [new Set(['sens']),new Set(['vie'])]; if(has('but')&&existence)return [new Set(['but']),new Set(['vie'])]; return []; };
