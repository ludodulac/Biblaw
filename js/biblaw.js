(() => {
  ...
  const activateThemeMode = () => { state.mode='themes'; $('modeThemes').classList.add('active'); $('modeExact').classList.remove('active'); $('primarySearch').classList.remove('exact-mode'); $('query2').disabled=false; $('modeHelp').textContent='Recherchez un thème indexé dans les psaumes. Un numéro de psaume peut aussi être saisi directement.'; $('query').placeholder='Ex. amour, nature, âme, psaume 105'; };
  const activateExactMode = () => { state.mode='exact'; $('modeExact').classList.add('active'); $('modeThemes').classList.remove('active'); $('primarySearch').classList.add('exact-mode'); $('query2').disabled=true; $('modeHelp').textContent='Recherchez les passages où un mot ou une expression apparaît réellement dans le texte.'; $('query').placeholder='Ex. frapper à la porte, lumière, eau'; hideAmbiguity(); hideTransverseNavigation(); };
  ...
  function render(items,indexedThemes=null,companion=null){
    ...
    $('results').innerHTML=unresolvedNotice+textualNotice+items.map(({record:r,thematic,matchedThemes,numberLookup,themeMatches,occurrenceCount})=>{
      ...
      const status=companion?.exactSearch?`${occurrenceCount} occurrence${occurrenceCount>1?'s':''}`:themeMatches?'Deux thèmes':thematic?`Importance : ${importanceLabel(thematic.importance)}`:numberLookup?'Numéro':companion?.textualFallback?'Occurrence du terme':'Texte';
