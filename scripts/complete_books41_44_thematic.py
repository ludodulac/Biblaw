#!/usr/bin/env python3
"""Build canonical thematic indexes for final books 41-44 from extracted PDF corpus only."""
import json,re
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]; CORPUS=ROOT/'data/corpus/books'; OUT=ROOT/'data/thematic-index/books'
THEMES=[('lumiere','Lumière',r'\blumi[eè]re'),('vie','Vie',r'\bvie\b'),('mort','Mort',r'\bmort'),('pensee','Pensée',r'pens[ée]'),('creation','Création',r'cr[ée](?:ation|er|atrice|ateur)'),('parole','Parole',r'\bparoles?\b'),('acte','Acte',r'\bactes?\b'),('oeuvre','Œuvre',r'\bœuvres?\b|\boeuvres?\b'),('corps','Corps',r'\bcorps'),('ame','Âme',r'\bâme'),('esprit','Esprit',r'\besprit'),('conscience','Conscience',r'\bconscience|conscient'),('intelligence','Intelligence',r'\bintelligence'),('sagesse','Sagesse',r'\bsagesse'),('verite','Vérité',r'v[ée]rit'),('purete','Pureté',r'pur(?:e|et[ée]|ifier|ification)'),('discernement','Discernement',r'\bdiscernement'),('responsabilite','Responsabilité',r'responsab'),('destinee','Destinée',r'destin[ée]e'),('liberte','Liberté',r'\blibert[ée]'),('maitrise','Maîtrise',r'ma[iî]tris'),('education','Éducation',r'[ée]ducat'),('tradition','Tradition',r'\btradition'),('alliance','Alliance',r'\balliance'),('loi','Loi',r'\blois?\b'),('bien-commun','Bien commun',r'bien commun'),('service','Service',r'\bservice'),('union','Union',r'\bunion\b|\bunir\b|unifi'),('harmonie','Harmonie',r'harmoni'),('equilibre','Équilibre',r'[ée]quilibr'),('stabilite','Stabilité',r'stabil'),('terre','Terre',r'\bterre'),('eau','Eau',r'\beau\b|eaux'),('air','Air',r'\bair\b'),('feu','Feu',r'\bfeu\b|flamme'),('mere','Mère',r'\bmère'),('pere','Père',r'\bpère'),('parents','Parents',r'\bparents?\b|parental'),('enfant','Enfant',r'\benfants?\b'),('famille','Famille',r'\bfamille'),('anges','Anges',r'\banges?\b'),('archanges','Archanges',r'\barchanges?\b'),('dieux','Dieux',r'\bdieux?\b'),('regnes','Règnes',r'\br[eè]gnes?\b'),('nature','Nature',r'\bnature\b'),('regard','Regard',r'\bregard'),('vision','Vision',r'\bvision\b|voir|regarder'),('oeil','Œil',r'\bœil\b|\boeil\b|\byeux\b'),('respiration','Respiration',r'respir'),('sentiments','Sentiments',r'\bsentiments?\b'),('volonte','Volonté',r'\bvolont[ée]'),('energie','Énergie',r'[ée]nergie'),('argent','Argent',r'\bargent\b'),('temps','Temps',r'\btemps\b'),('memoire','Mémoire',r'm[ée]moire'),('heredite','Hérédité',r'h[ée]r[ée]dit|lign[ée]e|g[ée]n[ée]rations?'),('immortalite','Immortalité',r'immortalit'),('eternite','Éternité',r'[ée]ternit'),('influences','Influences',r'\binfluenc'),('illusion','Illusion',r'illusion'),('concentration','Concentration',r'concentr'),('travail','Travail',r'\btravail|travaill'),('formation','Formation',r'\bformation'),('initiation','Initiation',r'initiat'),('nation-essenienne','Nation Essénienne',r'nation ess[ée]nienne'),('ronde-des-archanges','Ronde des Archanges',r'ronde des archanges'),('monde-divin','Monde divin',r'monde divin'),('monde-invisible','Monde invisible',r'monde invisible|mondes invisibles'),('monde-visible','Monde visible',r'monde visible|mondes visibles'),('transmission','Transmission',r'transm'),('protection','Protection',r'protect'),('guerison','Guérison',r'gu[ée]ri'),('vertu','Vertu',r'\bvertus?\b'),('paix','Paix',r'\bpaix\b'),('priere','Prière',r'\bpri[eè]re'),('temple','Temple',r'\btemple'),('religion','Religion',r'\breligion'),('arbre','Arbre',r'\barbres?\b'),('fleur','Fleur',r'\bfleurs?\b'),('animaux','Animaux',r'\banimaux\b|\banimal')]
...
def analyze(p):
 ...
 out={'recordId':p['id'],'number':p['number'],'title':p.get('title',''),'titleSignals':[x[1].lower() for x in c[:5]],'themes':themes}
 if p.get('noteIds'):out['notesUsed']=p['noteIds']
 return out
def build(n):
 meta=load(CORPUS/f'book-{n:02d}'/'book.json'); aa=[]
 for rid in meta.get('psalmIds',[]):
 ...
 majors=[k for k,_ in sorted(counts.items(),key=lambda kv:(-kv[1],kv[0]))[:20]]; complete=bool(aa) and not missing
...
def main():
 ...
 for n in range(41,45):
  if not (CORPUS/f'book-{n:02d}'/'book.json').exists(): continue
